...
import {
  getNavigatorFunctions,
  getPlugins,
  getSafeArea,
} from "./stages/navigator";
import {
  isAndroid,
  isChromium,
  isChromium86OrNewer,
...
import {
  getWebGLContext,
  getWebGlBasics,
} from "@/../../fingerprintjs/src/sources/webgl";
...
export function Fingerprint() {
  ...
  type Sources = {
    [key in keyof Fingerprint]:
      | Fingerprint[key]
      | (() => Promise<Fingerprint[key]> | Fingerprint[key]);
  };
  ...
  useLayoutEffect(() => {
    (async () => {
      ...
      const sources = {
        ...
        element_attributionSourceId: () =>
          document.createElement("a").attributionSourceId,
        element_attributeNames: () =>
          document.documentElement.getAttributeNames(),
        drm: detectSupportedDRMs,
        dom_blocker: getDomBlockers,
        canvas_geometry: x64hash128(canvas?.geometry),
        canvas_text: x64hash128(canvas?.text),
        ...
        navigator_media_devices: () =>
          navigator.mediaDevices.enumerateDevices() as any,
        ...
        storage_getDirectory: async () =>
          (await navigator.storage?.getDirectory()).name,
    ...
    <div className="min-h-[100vh] bg-gradient-to-b from-[#120b1c] to-[#15162c]">
      <main className="flex min-h-screen flex-col items-center justify-center px-5 text-center text-white">
        ...
        {insertFingerprint.isError ? (
          <h2 className="text-[5rem] font-semibold text-red-500">
            Error: {insertFingerprint.error.message}
          </h2>
        ) : (
          <>
            {" "}
            ...
            >
              <h2 className="z-10 w-full break-words bg-gradient-to-br from-purple-500 to-purple-700 bg-clip-text font-mono text-[2rem] font-semibold text-transparent sm:text-[3rem]">
                {insertFingerprint.data?.fingerprint ||
                  `0c7d1984f9d0b5040ff64321bf5b24ef`}
              </h2>
            </Skeleton>
            <h1
